...
function RecordDot() {
  ...
  return (
    <Animated.View
      style={[styles.recDot, useAnimatedStyle(() => ({ opacity: opacity.value }))]}
    />
  );
}
...
const WaveformBar = React.memo(function WaveformBar({
  ...
  useEffect(() => {
    height.value = withSpring(isRecording ? Math.max(2, amplitude * 68) : 2, {
      damping: 20,
      stiffness: 260,
    });
    opacity.value = withTiming(
      isRecording ? Math.max(0.12, (amplitude * 0.75 + 0.15) * centerBias) : 0.1,
      { duration: 120 },
    );
  ...
  return (
    <Animated.View
      style={[styles.bar, useAnimatedStyle(() => ({ height: height.value, opacity: opacity.value }))]}
    />
  );
});
...
export default function RecorderScreen() {
  ...
  if (screenState === 'denied') {
    return (
      <View style={[styles.container, styles.centered, { backgroundColor: colors.background, paddingTop: pt }]}>
